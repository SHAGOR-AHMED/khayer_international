...
class Currency {
	...
	function translate_to_words($number) {
		...
		if (is_int($number) && $number < abs($max_size)) {
			...
			switch ($number) {
				...
				case $number < 0:
					//$prefix = '<b class="text-danger">(Payable)</b>';
	...
	function get_bd_amount_in_text($amount) {
		...
		$output_string = '<span style="font-weight:bold;">In Words : </span>' . $p . ucwords($output_string);
		return $output_string;
	}

	function ladger_in_text($amount) {
		$sign = '';
		if ($amount < 0) {
		...
		if ($sign == '') {
			$prefix = '<b class="text-success">(Receivable)</b> ';
		} else {
			$prefix = '<b class="text-danger">(Payable)</b> ';
		}

		$output_string = $prefix . $output_string . $fraction . ' only';
		$output_string = '<span style="font-weight:bold;">In Words : </span>' . ucwords($output_string);
		return $output_string;
	}
	...
	/* $obj = new Currency();
	  ...
	  echo $obj->get_bd_amount_in_text($total);
	  }else{
	  echo $obj->get_bd_amount_in_text($total);
	  } */
	
	function tender_in_text($amount) {
		$sign = '';
		if ($amount < 0) {
			$amount = substr($amount, 1);
			$sign = '-';
		...
		if ($sign == '') {
			...
			$prefix = '<b class="text-danger">(Negative)</b> ';
		}
