...
<head>
    ...
    <style>
        ...
        body {
            font-size: 12px;
            font-family: SolaimanLipi, Nikosh, Arial, Helvetica, sans-serif;
        }
...
<body>
    <img src="https://new.akhayerintl.com/admin/img/banner.jpg" width="100%" height="100px;"
    ...
    <table class="table table-bordered report" border="2px">
        ...
        <tbody>
            ...
            if (count($results) == 0) {
                if ($is_all_banks) {
                    foreach ($all_banks as $bank) {
                        ...
                        echo "<td valign='top'>" . $bank->bank_name . '</td>';
                        echo "<td valign='top' colspan='3' class='text-right'> Current Balance = </td>";
                        echo "<td valign='top' class='text-right'>" . bd_money_format($balance) . '</td>';
                        echo '</tr>';
                    }
                } else {
                    echo '<tr>';
                    echo "<td valign='top' colspan='" . ($is_all_banks ? '6' : '5') . "' class='text-right'> Current Balance = </td>";
                    $balance = $bank->account_balance;
                    echo "<td valign='top' class='text-right'>" . bd_money_format($balance) . '</td>';
                    echo '</tr>';
                }
            ...
            if ($is_all_banks) {
                // Group results by bank for all banks report
                $grouped_results = [];
                foreach ($results as $row) {
                    $grouped_results[$row->bank_id][] = $row;
                }

                foreach ($grouped_results as $bank_id => $bank_results) {
                    $current_bank = \App\Models\Bank::find($bank_id);
                    $balance = 0;
                    $bank_row_count = 0;
                    ...
                    foreach ($bank_results as $row) {
                        ...
                        if ($bank_row_count == 1 && $row->transaction_type == 'Initial Balance') {
                            echo '<tr>';
                            echo "<td valign='top'></td>";
                            echo "<td valign='top'>" . $current_bank->bank_name . '</td>';
                            echo "<td valign='top' colspan='3' class='text-right'> Initial Balance = </td>";
                            $balance = $row->amount;
                            echo "<td valign='top' class='text-right'>" . bd_money_format($balance) . '</td>';
                            echo '</tr>';
                        } else {
                            if ($bank_row_count == 1) {
                                // Get previous balance for this bank
                                $balance = app(\App\Http\Controllers\Admin\Bank\IndexController::class)->previous_blance($bank_id, $from_date);
                                echo '<tr>';
                                echo "<td valign='top'></td>";
                                echo "<td valign='top'>" . $current_bank->bank_name . '</td>';
                                echo "<td valign='top' colspan='3' class='text-right'> Previous Balance = </td>";
                                echo "<td valign='top' class='text-right'>" . bd_money_format($balance) . '</td>';
                                echo '</tr>';
                            }
                            ...
                            } elseif ($row->transaction_type == 'Received') {
                ...
                if (count($results) > 0) {
                    echo '<tr>';
                    echo "<td valign='top' colspan='6' class='text-right'> Grand Total = </td>";
                    echo "<td valign='top' class='text-right'>" . bd_money_format($total_balance) . '</td>';
                    echo '</tr>';
                }
            } else {
                // Single bank report logic (existing)
                foreach ($results as $row) {
                    $i++;
            ...
            $obj = new Currency();
            if ($is_all_banks) {
                echo "<td valign='top' colspan='" . ($is_all_banks ? '7' : '6') . "' class='text-center'>" . $obj->ladger_in_text($total_balance) . '</td>';
            } else {
                if ($bank->bank_name == 'CASH') {
                    echo "<td valign='top' colspan='6' class='text-center'>" . $obj->get_bd_amount_in_text($total_balance) . '</td>';
                } else {
                    echo "<td valign='top' colspan='6' class='text-center'>" . $obj->ladger_in_text($total_balance) . '</td>';
                }
            }
            echo '</tr>';
            ?>
